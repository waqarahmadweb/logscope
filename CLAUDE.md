...
1. Agent suggests branch name; user creates it (`git checkout -b`) and confirms it's checked out.
2. **Agent waits for that confirmation before doing any file writes, installs, or build runs.** Read-only exploration on `main` is fine. Do not rely on "uncommitted changes carry over" as a substitute for branching first — that pattern is sloppy and loses the branch-gating benefit.
3. Agent does step work on the branch.
4. Agent drafts commit message; user commits + pushes + opens PR.
5. User merges (squash preferred) once CI is green.
