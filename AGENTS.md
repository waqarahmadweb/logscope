...
-   **WordPress 6.2+**
-   **Composer** with PSR-4 autoloading — namespace `Logscope\` mapped to `src/`. _(Added in commit 2.)_
...
-   **Path traversal prevention** — `PathGuard` class. Custom log paths MUST resolve to an allowlisted set of directories (WP root, `wp-content`, `ABSPATH`). Reject symlinks that escape. Reject `..` explicitly _before_ calling `realpath`. Test with adversarial inputs.
...
-   No comments that explain _what_ code does — only _why_, and only when the why is non-obvious.
...
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| 0.1.0 ✅         | Scaffold.                                                                                                                             |
| 0.2.0 → 0.9.0 ✅ | Tooling → bootstrap → parsers → REST → settings backend → admin/React shell → filters/grouping/tail → settings UI → polish/a11y/i18n. |
| 0.10.0           | Alerts (email + webhook + dedup).                                                                                                     |
| 0.11.0           | Scheduled fatal-log scanner (cron).                                                                                                   |
| 0.12.0           | Retention, mute, filter presets.                                                                                                      |
| 0.13.0           | Stats dashboard.                                                                                                                      |
| 0.14.0           | Onboarding, diagnostics, bulk actions.                                                                                                |
| 0.15.0           | wp.org release infrastructure (readme.txt, banner / icon / screenshots, release workflow).                                            |
| 0.x.0            | Pre-1.0 changes — open placeholder.                                                                                                   |
| **1.0.0**        | 🚀 **wp.org submission.** Gated by a full `security-review` skill pass.                                                               |
| 1.1.0+           | Live streaming, multisite, source preview, request context, etc.                                                                      |

Pre-1.0 bumps are **git tags only**. The wp.org release line is **v1.0.0 and only v1.0.0**.

> **Roadmap revision (2026-04-30):** Alerts and the cron scanner were pulled forward from post-1.0 (v1.1.0 / v1.2.0) into Phases 12 / 13 so the wp.org cut lands with a usable feature set. Phases 14–16 (retention, stats, onboarding) were added on the same revision. The original "Phase 12 = release infrastructure" content moved to Phase 17. See [ROADMAP.md](ROADMAP.md) for the full breakdown.

---

## 13. Docs you must update with every change

Shipping code without updating the surrounding docs creates drift fast. **Every commit that is worth a checkbox in the roadmap** must update the relevant docs _in the same commit_:
