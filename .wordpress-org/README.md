...
| `screenshot-1.jpg`    | ≥ 1280 px wide | JPEG              | Caption: "Log viewer (All entries) with severity filters and regex search."      |
| `screenshot-2.jpg`    | ≥ 1280 px wide | JPEG              | Caption: "Stats dashboard — severity breakdown, sparkline grid, top signatures." |
| `screenshot-3.jpg`    | ≥ 1280 px wide | JPEG              | Caption: "Unique errors view with bulk actions (mute / export CSV)."             |
| `screenshot-4.jpg`    | ≥ 1280 px wide | JPEG              | Caption: "Alerts settings — email and webhook configuration with test send."     |
| `screenshot-5.jpg`    | ≥ 1280 px wide | JPEG              | Caption: "Mute panel for unmuting silenced signatures."                          |
...
> **Re-capture required before the v1.0.0 cut:** the shipped screenshots predate the
> toolbar rename (they still show `Tail` / `Grouped` / `List`) and the old order
> (Grouped view was #2, Stats #3). Re-shoot all six on the current UI — the toolbar
> now reads **All entries / Unique errors / Live** — with Stats as screenshot 2.
...
-   **Filename order:** `screenshot-1.jpg` through `screenshot-6.jpg`. wp.org's renderer pairs each one with the matching numbered caption in `readme.txt`'s `== Screenshots ==` block. Adding a screenshot means adding both the file and a caption line; reordering means renaming files **and** caption lines together or the captions de-sync.
